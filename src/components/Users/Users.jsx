...
export const Users = ({ users, isLoading, onChangeSearchValue, searchValue, invitationList, onClickInvite,onClickSentInvites }) => {
        ...
        <ul className="users-list">
          {
            users.filter(user => {
              ...
              <User key={user.id} user={user} isInvited={invitationList.includes(user.id)} onClickInvite={onClickInvite}/>
            ))
          }
        </ul>
      )}
      {invitationList.length > 0 && <button onClick={
          onClickSentInvites()
